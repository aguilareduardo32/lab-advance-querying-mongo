...
sort: {number_of_employees: 1}
limit:10
db.companies.find({$and:[{number_of_employees: { $lte:1000 }}, {founded_year:{$lt:2005}}]}).sort({number_of_employees:1}).limit(10)
...
db.companies.find({number_of_employees:{$gte:100}},{number_of_employees:{$lte:1000}},{name:1,number_of_employees:1,id:0})
//### 9. Order all the companies by their IPO price in a descending order.
sort: {"ipo.valuation_amount":-1}
db.companies.find( { "ipo.valuation_amount": { $ne: null}}).sort( { 'ipo_valuation_amount': -1 })
...
db.companies.find({$and:[{founded_year:{$lt:200}},{"acquisitions.price_amount":{$gt:10000000}}]})
...
db.companies.find({$and:[{'acquisition.acquired_year':{$gte:2010}},{'acquisition.price_amount':{$ne:null}}]},{name:1, acquisition:1, _id:0}).sort({'aquisition.price_amount':-1})
...
db.companies.find( { founded_day: { $lte : 7, $gt : 1} },{ _id: 0, name: 1, founded_day: 1},).sort({'acquisition.price_amount': 1}).limit(100)
//### 16. All the companies on the 'web' `category` that have more than 4000 employees. Sort them by the amount of employees in ascending order.

query: {category_code:"web", number_of_employees:{$gt:4000}}
sort:{number_of_employees: 1}

db.companies.find({ $and: [{category_code:"web"},{number_of_employees:}]})
